  ...
  useEffect(() => {
    ...
    pRetry(() => ensure(), {
      ...
      onFailedAttempt: (error) => {
        console.warn("Failed to ensure Convex user, retrying", {
      ...
      .then(() => {
        ...
        console.log("Convex user ready")
      })
      .catch((error) => {
        console.error("Failed to ensure Convex user after all retry attempts", error)
        // Still allow UI to proceed (graceful degradation)
      })
      .finally(() => {
        onReady()
      })
  }, [ensure, onReady, userId])
...
function AuthToastWatcher() {
  ...
  useEffect(() => {
    ...
    if (previousIsSignedIn === false && isSignedIn === true) {
      notify({
        type: "auth.signedIn",
        level: "success",
